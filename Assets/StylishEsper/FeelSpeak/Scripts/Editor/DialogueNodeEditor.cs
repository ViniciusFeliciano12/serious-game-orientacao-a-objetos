        ...
        public override void Refresh()
        {
            ...
            if (node.character)
            {
                ...
                if (!node.character.sprite)
                {
                    if (!noCharacterImage)
                    {
                        noCharacterImage = AssetSearch.Find<Texture2D>("FeelSpeak", "Editor/Icons/character_icon.png");
                    }
            ...
            if (node.emotion)
            {
                ...
                if (!node.emotion.sprite)
                {
                    if (!noCharacterImage)
                    {
                        noEmotionImage = AssetSearch.Find<Texture2D>("FeelSpeak", "Editor/Icons/emotion_icon.png");
                    }
